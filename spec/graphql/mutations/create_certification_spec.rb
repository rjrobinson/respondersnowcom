...
describe Mutations::CreateCertification, type: :mutations do
  ...
  # set user
  subject(:create_certification) do
  ...
  let(:mutation_type) { "createCertification" }
  ...
  context "a user can request a certification" do
    let(:variables) do
      { certificationInput: {
        abbvr: "example",
        active: false,
        course_code: "EX",
        name: "exmaple certification"

      } }
    end

    it { is_expected.to include(certification: { id: be_a_kind_of(String), name: "exmaple certification" }) }
  end
end
